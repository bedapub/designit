...
#' @keywords internal
mk_autoscale_function = function(batch_container, random_perm) {
  ...
  message("MEANS:", paste(mu, collapse=", "))
  message("STDDEVS:", paste(sds, collapse=", "))
...
#' @param autoscale_scores Logical: if TRUE, perform boxcox transformation on the fly to equally scale scores
...
optimize_design <- function(batch_container, samples = NULL, n_shuffle = NULL,
                            ...
                            autoscale_scores = FALSE, autoscaling_permutations = 100,
  ...
  if (autoscale_scores && score_dim>1) {
    ...
    autoscale_func = mk_autoscale_function(batch_container$copy(), random_perm = autoscaling_permutations)
  } else {
