...
form_homogeneous_subgroups = function(batch_container, allocate_var, keep_together_vars=c(),
  ...
  if (length(use_vars)<length(keep_together_vars)) {
    warning("Ignoring 'keep_together_vars' not found in sample table: ",
            str_c(setdiff(keep_together_vars, use_vars), collapse=", "))
  }
  ...
  assertthat::assert_that(n_min<=n_max, msg=str_c("n_min (",n_min,") must not be greater than n_max (",n_max,")."))
  ...
  assertthat::assert_that(n_ideal<=n_max, n_ideal>=n_min,
                          msg=str_c("n_ideal (", n_ideal, ") must be between n_min (",n_min,") and n_max (",n_max,")."))
  ...
  list(Grouped_Samples = grouped_samples, Subgroup_Sizes = subgroup_sizes,
       Allocate_Var = allocate_var, Allocate_Levels=levels(allocate_fac),
       Subgroup_Var_Name = subgroup_var_name,
       options = list( n_min=n_min, n_max=n_max, n_ideal=n_ideal,
...
mk_subgroup_shuffle_function = function(subgroup_object, subgroup_allocations,
  ...
  subgroup_vec = rep.int(  unlist(purrr::map(subgroup_object$Subgroup_Sizes, seq_along)),
                           unlist(subgroup_object$Subgroup_Sizes) )

  fullgroup_vec = str_c(group_vec, subgroup_vec, sep="_")
  ...
  function(onlyShuffleIndex = TRUE, bufferedShuffle = FALSE, ...) {
    ...
    app = tibble(alloc_var_level=subgroup_object$Allocate_Levels[alloc_var_assigned[shuffle_index] ],
...
shuffle_grouped_data = function(batch_container, allocate_var,
  ...
  sg = form_homogeneous_subgroups(batch_container = batch_container, allocate_var=allocate_var,
