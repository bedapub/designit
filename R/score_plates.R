...
mk_plate_scoring_functions = function (batch_container, plate=NULL, row, column, group) {
  ...
  plate_scoring_func = function(samples, plate, plate_name, row, column, group) {
    ...
    assertthat::assert_that(!all(is.na(samples[[group]][nonempty_pos])), msg=stringr::str_c("Group variable for plate ", plate_name," must not be all NAs" ))

    # Have to remember ALL levels of the Group factor since sample exchanges may happen across plates, yielding different groups on the plate each time.
