...
test_that("n_shuffle could be >=1 for shuffling with proposal function", {
  expect_error(optimize_design(bc, max_iter = 10, n_shuffle = 1, shuffle_proposal = my_shuffle_proposal),
               NA)
  expect_error(optimize_design(bc, max_iter = 10, n_shuffle = rep(c(3, 1), c(5, 5)), shuffle_proposal = my_shuffle_proposal),
               NA)
  expect_error(optimize_design(bc, max_iter = 10, n_shuffle = 10, shuffle_proposal = my_shuffle_proposal),
               NA)
...
test_that("mismatch between number of iterations and n_shuffle", {
  expect_error(trace <- optimize_design(bc, max_iter = 5, n_shuffle = 2),
               NA)
  expect_equal(trace$n_steps, 5 + 1)
  expect_error(trace <- optimize_design(bc, max_iter = 10, n_shuffle = c(2, 2)),
               NA)
  expect_equal(trace$n_steps, 2 + 1)
  expect_error(trace <- optimize_design(bc, max_iter = 10, n_shuffle = rep(2, 200)),
               NA)
  expect_equal(trace$n_steps, 10 + 1)
})

test_that("inferring iterations from n_shuffle", {
  expect_error(trace <- optimize_design(bc, n_shuffle = rep(2, 5)),
               NA)
  expect_equal(trace$n_steps, 5 + 1)
})

test_that("default n_shuffle", {
  expect_error(optimize_design(bc, max_iter = 10),
               NA)
  expect_error(optimize_design(bc, max_iter = 10, shuffle_proposal = my_shuffle_proposal),
               NA)
})
