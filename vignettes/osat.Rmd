---
title: "OSAT and scoring function"
...
```{r}
# scores using tidy implementation
OSAT::getLayout(gc) %>%
...
OSAT::getLayout(gc) %>%
  ...
  osat_score_datatable("plates", c("SampleType", "Race", "AgeGrp")) %>%
  .$score
...
## Simple OSAT score implementation
```{r}
bc$scoring_f <- function(bc) osat_score(bc$get_samples(), plates, c(SampleType, Race, AgeGrp))$score
...
# precompute expected_df
edf <- osat_score(bc$get_samples(), plates, c(SampleType, Race, AgeGrp))$expected_df

bc$scoring_f <- function(bc) osat_score(bc$get_samples(), plates, c(SampleType, Race, AgeGrp), expected_df = edf)$score
...
### Using `data.table` implementation
```{r}
bc$assignment_vec <- starting_assignment

# precompute expected_df
edf <- osat_score_datatable(data.table::data.table(bc$get_samples()), "plates", c("SampleType", "Race", "AgeGrp"))$expected_df

bc$scoring_f <- function(bc) osat_score_datatable(data.table::data.table(bc$get_samples()), "plates", c("SampleType", "Race", "AgeGrp"), expected_df = edf)$score
...
fast_osat_optimize <- function(bc, batch_vars, feature_vars, iterations) {
  ...
  v <- osat_score_datatable(df, batch_vars, feature_vars)
  edf <- v$expected_df
  ...
  for (i in 1:iterations) {
    ...
    new_score <- osat_score_datatable(df, batch_vars, feature_vars, edf)$score
...
bc$aux_scoring_f <- list(
  function(...) rnorm(1, 100),
  function(...) -7
)

bc$scoring_f <- function(bc) rnorm(1) + 2 * rexp(1)

bc$aux_scoring_f <- list(
  fc1 = function(...) rnorm(1, 100),
  fc2 = function(...) -7
)
