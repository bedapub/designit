...
invivo_samples = mutate(invivo_samples, Litter_alloc = ifelse(Sex=="F","female_all",Litter)) %>%
  arrange(Genotype)
...
#bc$scoring_f <- list( function(samples) { osat_score(samples, batch_vars = "Treatment", feature_vars = c("Genotype"))$score },
#                      function(samples) { osat_score(samples, batch_vars = "Treatment", feature_vars = c("Sex"))$score }
#                      )
...
subg = form_homogeneous_subgroups(batch_container = bc, allocate_var="Treatment",
...
`mk_subgroup_shuffle_function()` with the previously created subgrouping object and the list of possible treatment allocations. 

Every call to this shuffling function will return a permutation index (of the original samples) that constitues a valid solution
to be scored.
...
count(design, Treatment, Genotype, Sex) %>% gt::gt()

count(design, Treatment, Genotype) %>% gt::gt()

count(design, Treatment, Sex) %>% gt::gt()
...
bc3$scoring_f <- list( function(samples) { osat_score(samples, batch_vars = "Treatment", feature_vars = c("Genotype", "Sex"))$score },
                       function(samples) { osat_score(samples, batch_vars = "Treatment", feature_vars = c("Bodyweight"))$score }
                      )

optimize_design(
  bc3,
  shuffle_proposal_func = shuffle_grouped_data(bc3, allocate_var="Treatment",
                                               keep_together_vars=c("Genotype", "Sex"),
                                               subgroup_var_name = "Cage",
                                               n_min=2, n_max=4),
...
design %>% 
  dplyr::mutate(Condition = levels(treatments)[as.integer(alloc_var_level)]) %>%
  gt::gt()
...
optimize_design(
  bc4,
  shuffle_proposal_func = shuffle_grouped_data(bc4, allocate_var="Treatment",
...
from the size constraints if needed. (Another option is to modify the size constraints directl of course!)
...
count(design, Condition, Genotype, Sex, Cage ) %>% gt::gt()
...
optimize_design(
  bc5,
  shuffle_proposal_func = shuffle_grouped_data(bc5, allocate_var="Treatment",
                                               keep_together_vars=c("Genotype", "Sex"),
...
count(design, Cage, Earmark ) %>% gt::gt()
