...
```{r}
bc$scoring_f <- function(samples) osat_score(samples, batch_vars = "plate", 
                                             feature_vars = c("treatment", "dose"))$score
```
...
```{r}
color_vector <- RColorBrewer::brewer.pal(length(unique(samples$treatment)),
                                         name = "Set1")
names(color_vector) <- unique(samples$treatment)
plot_plate(
