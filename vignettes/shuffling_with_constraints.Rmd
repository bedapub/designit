...
a little generic constructor for customized shufflings (\code{shuffle_grouped_data}) in which certain types of constraints
...
invivo_study_samples %>% dplyr::count(Strain, Sex, Litter_combine_females) %>% gt::gt()
...
The wrapper \code{shuffle_grouped_data} allows to construct a shuffling function that satisfies all constraints
...
design %>% dplyr::count(Cage, Strain, Sex, Treatment)  %>% gt::gt()
```
...
**report_grouping_as_attribute** allos, if TRUE, to add the updated group variable into the batch container at each
...
\code{shuffle_grouped_data} is a wrapper that consecutively calls other helper function. As an addendum, let us break
