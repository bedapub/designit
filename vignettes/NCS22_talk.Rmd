...
```{r, echo=FALSE}
bind_rows(head(bc$get_samples(), 3) %>% 
          ...
          tail(bc$get_samples(), 3) %>% 
            mutate(across(, as.character))) %>% 
  gt::gt() %>% gt::tab_options(table.font.size = 11, 
...
```{r fig.width=3, fig.height=3, echo=FALSE}
purrr::imap(traces, ~ .x$plot(include_aggregated=TRUE) + labs(title = .y))[1:2]
```
