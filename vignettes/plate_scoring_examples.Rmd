...
start_time <- Sys.time()
optimize_design(bc,
  min_score = 6.1, max_iter = 1e4, # this is set to shorten vignette run-time, normally we don't know.
  n_shuffle = 2,
  acceptance_func = mk_simanneal_acceptance_func(mk_simanneal_temp_func(T0 = 1000, alpha = 0.5)),
  quiet = TRUE
)
...
cowplot::plot_grid(
  plot_plate(bc$get_samples(), plate = plate, row = row, column = col, .color = Group, title="Initial layout by Group"),
  plot_plate(bc$get_samples(), plate = plate, row = row, column = col, .color = Sex, title="Initial layout by Sex"),
  ncol = 2
)
...
cowplot::plot_grid(
  plot_plate(bc$get_samples(), plate = plate, row = row, column = col, .color = Group, title="Final layout by Group"),
  plot_plate(bc$get_samples(), plate = plate, row = row, column = col, .color = Sex, title="Final layout by Sex"),
  ncol = 2
)
```
