...
In some cases it ma<y be essential to avoid samples of the same group being put into the same row or column
on a plate, i.e. these variables are regarded as factor levels of their own, in addition to the spatial relation
of the samples.

The plate_scoring functions can use an specific penalty for these 'linearly arranged' samples on top of the
distance metrics chosen. 


```{r include=FALSE, message = FALSE}
designit::.load_cache("trace15")
```
