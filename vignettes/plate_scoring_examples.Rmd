...
We do the  same example with auto-scaling and position-dependent scoring now, not aggregating the score vector!
This is more effective even when using the default acceptance function.
We are prioritizing the leftmost score in addition to reflect relevance for the design.
...
Testing an alternative left-to-right weighing of scores, based on exponential down-weighing of the
respective score differences at position p with factor kappa^p, 0<kappa<1 
We choose a kappa of 0.5, i.e. the second score's improvement counts half of that of the first one.
